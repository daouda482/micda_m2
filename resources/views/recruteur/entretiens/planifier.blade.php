    ...
    <form action="{{ route('recruteur.entretiens.store', $candidature->id) }}" method="POST">
                ...
                <select name="candidat_id" class="form-control" required>
                    ...
                    @foreach($candidats as $candidat)
                        <option value="{{ $candidat->id }}"
                            {{ $candidat->id == $candidature->candidat->id ? 'selected' : '' }}>
                            {{ $candidat->prenom }} {{ $candidat->name }}
                        </option>
                    @endforeach
                </select>
            </div>
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control" name="lieu"
                   placeholder="Ex: Salle 101 ou Dakar Plateau">
        </div>

        <div class="mb-3">
            <label class="form-label">Message</label>
            <textarea class="form-control" name="message" rows="3"
                      placeholder="Ex: Merci de vous présenter 10 minutes avant l'entretien."></textarea>
        </div>
